 ...
 * omega_daemon.cpp — Daemon de audio con PF Engine real (Biquad + waveshaping)
 *
 * Arquitectura:
 *   ┌────────────────────────────────────────────────────────┐
 *   │  APK (UI)  ──JNI──►  OmegaDaemon.kt                   │
 *   │                           │                            │
 *   │                      omega_daemon.cpp                  │
 *   │                           │                            │
 *   │           ┌───────────────┴──────────────────┐         │
 *   │     process_audio_thread()          socket_server_thread()
 *   │           │                                  │         │
 *   │     processPFEngine()              handleClientCommand()│
 *   │     (tanh drive + 4-band EQ)       (audioserver /      │
 *   │     ivanna::Biquad                  Magisk module)      │
 *   │           │                                            │
 *   │     OmegaSharedState (memfd SHM, SCM_RIGHTS)           │
 *   └────────────────────────────────────────────────────────┘
 */
...
#include "dsp_types.h"   // ivanna::Biquad, ivanna::DSPParams
...
// 4 filtros x 2 canales (interleaved stereo): low shelf, mid peak,
// high shelf, presence peak. Se recalculan cuando pf_param_version cambia.
struct PFEngineState {
    ...
    void recompute(const OmegaSharedState* s) {
        ...
        // Clamp Q para evitar inestabilidad
        if (Q < 0.1)  Q = 0.1;
        if (Q > 10.0) Q = 10.0;
...
// Buffer layout: interleaved stereo [L0, R0, L1, R1, …], frames = OMEGA_BLOCK_SIZE
__attribute__((hot, flatten))
void processPFEngine(float* __restrict__ buf, int frames) {
    if (!g_shared) return;

    // Recomputar coeficientes si los parámetros cambiaron
    ...
    // Leer parámetros escalares una sola vez (lecturas relaxed, estamos
    // en el hilo de audio — no necesitamos acquire aquí)
    ...
    float master_g = std::exp2f(g_shared->pf_master.load(std::memory_order_relaxed) * 0.1660964f); // dB→lin

    // Pre-gain para el drive: escala [1, 5] en el rango drive=[0,1]
    float drive_scale = 1.0f + drive * 4.0f;
    // Post-compensación: atenúa para mantener loudness percibido constante
    float drive_inv   = 1.0f / drive_scale;

    // Mezcla salida = mix * master_gain
    ...
    for (int i = 0; i < frames; ++i) {
        for (int ch = 0; ch < 2; ++ch) {
            ...
            // 1. Drive: pre-gain + tanh soft-clip + compensación de amplitud
            float driven = std::tanhf(in * drive_scale) * drive_inv;

            // 2. EQ 4 bandas (ivanna::Biquad)
            ...
            // 3. Wet/dry mix interno + ganancia de salida
...
bool init_shared_memory() {
    ...
    // Inicializar coeficientes Biquad con los valores default del constructor
    g_pf.recompute(g_shared);
...
void process_audio_thread() {
    ...
    while (__builtin_expect(g_running.load(std::memory_order_relaxed), 1)) {
        ...
        // Verificación térmica + AI auto-adapt cada 200 iteraciones
        ...
        // ── PF Engine real ────────────────────────────────────────────────
        if (__builtin_expect(g_complexity_level.load(std::memory_order_relaxed) < 2, 1)) {
            processPFEngine(g_process_buf, OMEGA_BLOCK_SIZE);
        }
        // En throttling severo (level=2) → bypass implícito: buf queda intacto
...
// ── Comandos del socket ────────────────────────────────────────────────────────
// Incrementa pf_param_version para que el hot-path recompute Biquad
...
void handleClientCommand(const std::string& cmd) {
    ...
    // PF Engine params — comandos para el módulo Magisk / audioserver
...
void socket_server_thread() {
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        while (g_running.load(std::memory_order_relaxed)) {
            ...
            timeout = { 0, 50000 };
            if (select(client + 1, &readfds, nullptr, nullptr, &timeout) <= 0) {
                if (!g_running.load(std::memory_order_relaxed)) break;
                continue;
            }
...
// ── JNI ── com.ivannafusion.OmegaDaemon ──────────────────────────────────────
extern "C" {
...
// Params que NO afectan coeficientes Biquad → sin bump de versión
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFDrive(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_drive.store(v, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFWet(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_wet.store(v, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFMix(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_mix.store(v, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFAlpha(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_alpha.store(v, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFBeta(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_beta.store(v, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFGamma(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_gamma.store(v, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFMaster(JNIEnv*, jobject, jfloat v) {
    if (g_shared) g_shared->pf_master.store(v, std::memory_order_relaxed);
}

// Params que SÍ afectan coeficientes Biquad → bump de versión
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFFreq(JNIEnv*, jobject, jfloat v) {
    if (!g_shared) return;
    g_shared->pf_freq.store(v, std::memory_order_relaxed);
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFResonance(JNIEnv*, jobject, jfloat v) {
    if (!g_shared) return;
    g_shared->pf_resonance.store(v, std::memory_order_relaxed);
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFLow(JNIEnv*, jobject, jfloat v) {
    if (!g_shared) return;
    g_shared->pf_low.store(v, std::memory_order_relaxed);
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFMid(JNIEnv*, jobject, jfloat v) {
    if (!g_shared) return;
    g_shared->pf_mid.store(v, std::memory_order_relaxed);
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFHigh(JNIEnv*, jobject, jfloat v) {
    if (!g_shared) return;
    g_shared->pf_high.store(v, std::memory_order_relaxed);
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFPresence(JNIEnv*, jobject, jfloat v) {
    if (!g_shared) return;
    g_shared->pf_presence.store(v, std::memory_order_relaxed);
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}

// ── Bulk setter — todos los 13 params de una vez (ruta preferida desde la UI) ──
JNIEXPORT void JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeSetPFParams(
        JNIEnv*, jobject,
    ...
    // Un solo bump cubre todos los cambios de esta llamada
    g_shared->pf_param_version.fetch_add(1, std::memory_order_relaxed);
}

// ── Getter bulk — para la UI / telemetría ────────────────────────────────────
JNIEXPORT jfloatArray JNICALL
Java_com_ivannafusion_OmegaDaemon_nativeGetPFParams(JNIEnv* env, jobject) {
