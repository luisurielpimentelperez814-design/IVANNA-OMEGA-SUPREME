...
int main(int argc, char* argv[]) {
    ...
    // Start TCP fallback listener (if requested) on loopback in a detached thread.
    if (tcp_port > 0) {
        try {
    ...
    //    (OmegaDaemon.kt mantiene su socket abierto de forma persistente), ningún otro podía
