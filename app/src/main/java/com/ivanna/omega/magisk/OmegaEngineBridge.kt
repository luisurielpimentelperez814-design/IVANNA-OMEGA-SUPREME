...
object OmegaEngineBridge {
    ...
    fun pushSAFState(deltaEnergy: Float, metricNorm: Float, memory: Float, gain: Float): Boolean =
        sendCommand(JSONObject().apply { put("action","PUSH_SAF_STATE"); put("deltaEnergy", deltaEnergy); put("metricNorm", metricNorm); put("memory", memory); put("gain", gain) })
    fun pushSAFState(json: JSONObject): Boolean = sendCommand(JSONObject().apply { put("action","PUSH_SAF_STATE"); put("state", json) })

    fun sendPerceptualState(compressor: Float, exciterRed: Float, highCut: Float, spatialWidth: Float, loudnessTarget: Float, harmonicGain: Float, antiDolby: Float): Boolean =
        sendCommand(JSONObject().apply { put("action","SEND_PERCEPTUAL_STATE"); put("compressor", compressor); put("exciterRed", exciterRed); put("highCut", highCut); put("spatialWidth", spatialWidth); put("loudnessTarget", loudnessTarget); put("harmonicGain", harmonicGain); put("antiDolby", antiDolby) })
    ...
    fun setRouteProfile(route: String, profile: String, intensity: Float): Boolean = sendCommand(JSONObject().apply { put("action","SET_ROUTE_PROFILE"); put("route", route); put("profile", profile); put("intensity", intensity) })

    fun pushYamnetScores(speech: Float, music: Float, classId: Int, confidence: Float): Boolean =
        sendCommand(JSONObject().apply { put("action","PUSH_YAMNET_SCORES"); put("speech", speech); put("music", music); put("classId", classId); put("confidence", confidence) })
