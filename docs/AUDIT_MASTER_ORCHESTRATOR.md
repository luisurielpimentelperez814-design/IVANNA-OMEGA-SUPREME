...
| **PerceptualBrainDashboard** | — | — | **NO EXISTE** en el árbol actual (ni por ese nombre ni variante cercana). Si el prompt lo asume construido, es incorrecto — el árbol gana | — |
...
5. **`PerceptualBrainDashboard` no existe** — si alguna fase posterior del prompt
   asume su existencia, hay que tratarlo como no construido.
