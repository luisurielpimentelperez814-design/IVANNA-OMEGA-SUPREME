...
if OLD_INTENT not in src:
    print("ERROR: anchor AudioForegroundService Intent no encontrado", le=sys.stderr)
    sys.exit(1)
...
if OLD_INTENT not in src:
    print("ERROR: anchor PlaybackCaptureService Intent no encontrado", le=sys.stderr)
    sys.exit(1)
...
git cong user.email "ivanna@goretnsfx.com" 2>/dev/null || true
git cong user.name "IVANNA-OMEGA" 2>/dev/null || true
...
git commit -m "x: romper dependencia circular MainActivity en servicios de audio
