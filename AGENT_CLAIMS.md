...
---
### Nota de coordinación — toma temporal de DSP nativo para cablear currentEqBoost() (instrucción directa del propietario, 2026-09-22)
**Contexto:** el propietario pidió tomar el flanco DSP nativo si era necesario para cerrar el hueco anotado en la nota anterior (`currentEqBoost()`/`currentExciterLowOnly()` calculados en AntiDolbyState pero sin consumidor real). Toma **temporal y acotada** — no reclamo el flanco completo, solo este punto puntual; sesión Claude activa desde 2026-09-07 sigue siendo la dueña del resto.

**Entregado:** `currentEqBoost()` (presencia vocal 2-4kHz) ahora cablea de verdad — biquad RBJ peaking dedicado en `omega_effect.cpp`, centro 3kHz/Q=1.2, coeficientes recalculados cada bloque a partir del dB ya suavizado por `AntiDolbyState::tick()` (rango real acotado en `anti_dolby.cpp`: 0 a +2dB × intensidad 0..1, sin riesgo de ganancia disparatada). Se procesa siempre (nunca se salta por boost≈0) a propósito: con A=1 el peaking RBJ es matemáticamente la identidad (b0=a0, b1=a1, b2=a2), así que saltar el bloque no ahorra nada real y sí introduce el mismo salto de rama que este repo ya peleó muchas veces en otros filtros. NaN-guard de entrada/salida igual que ParametricEQ/Biquad ya establecidos en el repo. Estado de memoria nuevo en el contexto (calloc, arranque en 0). Diff puramente aditivo (50+ líneas nuevas, cero líneas borradas).

**NO entregado, a propósito — `currentExciterLowOnly()`/`bassExciterLevel`:** verificado por lectura que no existe ningún exciter de graves dedicado en el repo (el único `HarmonicExciter` real está diseñado y afinado específicamente para agudos >3kHz vía HPF fijo — su anti-aliasing/oversampling está calibrado para ese caso, no para graves). Cablear esto de verdad requiere una unidad DSP nueva (no conectar dos piezas ya existentes como en el caso de arriba), sin poder verificarla de oído en este entorno (sin dispositivo físico). Fabricar esa pieza a ciegas ahora mismo es el mismo riesgo que este archivo lleva cientos de commits documentando y corrigiendo — queda pendiente, explícito, para quien retome DSP nativo con acceso a dispositivo real.

**Verificación:** por lectura únicamente (sin SDK/dispositivo en este entorno) — coherente sintácticamente con el resto del archivo, misma convención de guardas NaN y de nombres de variables que el bloque del widener inmediatamente anterior. Cedo el flanco de vuelta a la sesión Claude activa; no toqué nada más de `app/src/main/cpp/`.
